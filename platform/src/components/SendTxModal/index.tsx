...
import { POLYGON_PUBLIC_CLIENT, SEPOLIA_ETH_TESTNET_PUBLIC_CLIENT } from "@/constants";
  ...
  async function resolveUserInputDestination(value: string) {
    ...
    if (value.match(/^0x[a-fA-F0-9]{40}$/)) {
      ...
      try {
        ...
        const name = await SEPOLIA_ETH_TESTNET_PUBLIC_CLIENT.getEnsName({
          address: normalize(value) as Hash,
        });
        // const name = await POLYGON_PUBLIC_CLIENT.getEnsName({
        //   address: normalize(value) as Hash,
        // });
        if (name) {
    ...
    if (value.endsWith(".eth")) {
      setEnsIsLoading(true);
      try {
        const address = await SEPOLIA_ETH_TESTNET_PUBLIC_CLIENT.getEnsAddress({
          name: normalize(value),
        });
        // const address = await POLYGON_PUBLIC_CLIENT.getEnsAddress({
        //   name: normalize(value),
        // });
